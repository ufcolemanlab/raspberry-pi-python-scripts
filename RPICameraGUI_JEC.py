...
class ViewerPanel(wx.Panel):
    ...
    def fillCS(self):
        ...
        # JEC
        self.cbtl=wx.CheckBox(self.CS, -1, '-tl ', (xoffset, 470))
        ...
        # JEC
        self.sctl = wx.SpinCtrl(self.CS, -1, str(0), (xoffset+40, 470), (60, -1), min=100, max=50000)
        
        wx.Button(self.CS, 1, 'Take Photo', (30, 500)) #JEC 470(500)
        ...
        # JEC
        wx.StaticText(self.CS, -1,'time lapse x<ms>' , (xcomoffset,470))
    ...
    def TakePic(self, event):
        ...
        if self.cbo.GetValue() and (self.cbtl.GetValue() == 0):
            self.cmdln=self.cmdln + '-o "' + str(self.oname.GetValue())+'" '
        else :
            tempfilename = str(self.oname.GetValue())
            timelapse_filename = tempfilename.replace('.jpg','')
            self.cmdln=self.cmdln + '-o "' + timelapse_filename+ '%04d.jpg" ' 
        if self.cbq.GetValue() :
            self.cmdln=self.cmdln + '-q ' + str(self.scq.GetValue())+' '
        if self.cbtl.GetValue() :
            self.cmdln=self.cmdln + '-tl ' + str(self.sctl.GetValue())+' '
        ...
        if self.cbtl.GetValue() == 1 :
            defaultfilename = timelapse_filename + '_0001.jpg'
        else :
